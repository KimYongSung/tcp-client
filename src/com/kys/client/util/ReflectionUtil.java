...
public class ReflectionUtil{
	 ...
	 */
	private static final Map<Class<?>, Method[]> cacheMethodMap = new ConcurrentHashMap<Class<?>, Method[]>();
	
	/**
	 * Reflection 성능 향상을 위한  캐싱용 맵
	 * <br><br><strong> 서비스별 reflection을 사용하는 VO의 숫자는 일정하다는 가정하에 메모리 캐싱</strong>
	 */
	private static final Map<Class<?>, Field[]> cacheFieldMap = new ConcurrentHashMap<Class<?>, Field[]>();
	
	/**
	 * Reflection 성능 향상을 위한  캐싱용 맵
	 * <br><br><strong> 서비스별 reflection을 사용하는 VO의 숫자는 일정하다는 가정하에 메모리 캐싱</strong>
	 */
	private static final Map<Class<?>, Constructor<?>[]> cacheConstructorMap = new ConcurrentHashMap<Class<?>, Constructor<?>[]>();
	
	/**
	 * Reflection 성능 향상을 위한 캐싱용 맵
	 * <br><br><strong> 서비스별 reflection을 사용하는 VO의 숫자는 일정하다는 가정하에 메모리 캐싱</strong>
	 */
	private static final Map<AnnotatedElement, Annotation[]> cacheAnnotaionMap = new ConcurrentHashMap<AnnotatedElement, Annotation[]>();
	...
	public static boolean isCollectionGenericTypeEquals(Field field, SuperTypeToken<?> token){
		return isTypeEquals(field, Collection.class) && getGenericTypeName(field.getGenericType(), 0).equals(token.typeName());
	}
	...
	public static boolean isMapGenericTypeEquals(Field field, SuperTypeToken<?> key, SuperTypeToken<?> value){
		...
		return  getTypeName(genericTypes[0]).equals(key.typeName()) && getTypeName(genericTypes[1]).equals(value.typeName());
	}
	...
	public static Type getGenericType(Type type, int index){
		...
		if((actualTypeArguments.length-1) < index){
			throw new IllegalArgumentException("ActualTypeArgument index out of bounds - "+actualTypeArguments.length);
		}
	...
	public static <T> T newInstance(Class<T> clazz, Object... arg){
		try {
			return newInstance(findConstructor(clazz, findType(arg)), arg);
		} catch (Exception e) {
			handleReflectionException(e);
		}
		return null;
	...
	public static Method findMethod(Class<?> clazz, String name, Class<?>... paramTypes){
		...
		for (Method method : methods) {
			if(method.getName().equals(name) && paramTypes == null || Arrays.equals(paramTypes, method.getParameterTypes())){
				return method;
			}
		}
	 ...
	 * @return
	 */
	private static Class<?>[] findType(Object... arg){
	...
	private static <T> Constructor<T>[] getDeclaredConstructor(Class<T> clazz){
		Constructor<T>[] constructors = (Constructor<T>[]) cacheConstructorMap.get(clazz);
	...
	public static abstract class SuperTypeToken<T>{
		...
		public boolean equals(Object obj) {
			return (this == obj || (obj instanceof SuperTypeToken &&
					this.type.equals(((SuperTypeToken<?>) obj).type)));
		}
